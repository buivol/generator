<?php
	...
	foreach ($settings['config']['marks'] as $mark) {
		...
		if(!WINDOWS) $abs_path_win = str_replace('\\', '/', $abs_path_win);
		if(WINDOWS) $abs_path_win = iconv('UTF-8', 'windows-1251', $abs_path_win);
		if(file_exists($abs_path_win) && $mark['active'] == 'true'){
			...
			if(strlen($mark['image'])>1){
				...
				if(!WINDOWS) $abs_path_win_i = str_replace('\\', '/', $abs_path_win_i);
				if(WINDOWS) $abs_path_win_i = iconv('UTF-8', 'windows-1251', $abs_path_i);
			...
			if(strlen($mark['header'])>1){
				...
				$abs_path_win_i = $abs_path_i;
				if(!WINDOWS) $abs_path_win_i = str_replace('\\', '/', $abs_path_win_i);
				if(WINDOWS) $abs_path_win_i = iconv('UTF-8', 'windows-1251', $abs_path_win_i);
				if(file_exists($abs_path_win_i)){
					$m['header'] = $abs_path_i;
				}
			} else {
